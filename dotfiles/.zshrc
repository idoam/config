...
configure_prompt() {
    ...
    PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n'$prompt_symbol$'%m%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}] '$git$'\n└─%B%(#.%F{red}#.%F{blue}42sh$)%b%F{reset} '
...
alias glog='git log --compact-summary'
...
export NNTPSERVER='snews://news.epita.fr'
alias slrn="slrn -p 563"
